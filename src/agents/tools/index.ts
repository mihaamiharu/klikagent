...
import { writerDiscoveryTools, createWriterDiscoveryHandlers } from './writerTools';
import {
...
// Writer: validate + done + discovery tools (Phase 1)
// Discovery tools give on-demand access to the test repo filesystem.
export const writerTools: AgentTool[] = [validateTypescriptTool, qaDoneTool, ...writerDiscoveryTools];
...
export function createWriterHandlers(repoName: string): ToolHandlers {
  // Writer gets TypeScript validation + on-demand repo discovery (Phase 1)
  return { ...validateTypescriptHandler, ...createWriterDiscoveryHandlers(repoName) };
}
